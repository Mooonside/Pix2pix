...
class defineD_pixelGAN(HybridBlock):
    ...
    """
    def __init__(self, input_nc, output_nc, ndf, norm_layer=nn.BatchNorm, norm_kwargs=None):
...
class defineD_n_layers(defineD_pixelGAN):
    ...
    """
    def __init__(self, input_nc, output_nc, ndf, n_layers=3, norm_layer=nn.BatchNorm, norm_kwargs=None):
        if n_layers == 0:
            super(defineD_n_layers, self).__init__(input_nc, output_nc, ndf, norm_layer, norm_kwargs)
        else:
            super(defineD_pixelGAN, self).__init__()
            if norm_kwargs is None:
                norm_kwargs = {}
